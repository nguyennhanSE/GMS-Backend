...
class GetClassSchedulesQueryDto {
  page?: string;
  limit?: string;
  sort?: 'asc' | 'desc';
  sortBy?: string;
...
export class ClassScheduleController {
  ...
  @ApiResponse({ status: 400, description: 'Bad request - validation error or class schedule already exists' })
  async create(@Body() createClassScheduleDto: CreateClassScheduleDto) {
    const responseModel = new ResponseModel();

    try {
      const classSchedule = await this.classScheduleService.create(createClassScheduleDto);
  ...
  async update(
    ...
    try {
      const classSchedule = await this.classScheduleService.update(id, updateClassScheduleDto);
      const result = toResponse(classSchedule);
      responseModel.setData(result);
    } catch (error) {
      throw error;
